...
export class AcpClient extends EventEmitter {
  constructor({ cwd, env, modelHint, settings } = {}) {
    super();
    this.cwd = cwd;
  ...
  // Build the argv passed to `grok` based on the per-agent settings object.
  // Result layout: [<top-level flags>, 'agent', '--no-leader', ...<agent flags>, 'stdio'].
  //
  // Settings keys honored (all optional):
  //   model:                -m <model>           (top-level)
  //   reasoningEffort:      --reasoning-effort <effort>   (top-level)
  //   systemPromptOverride: --system-prompt-override <text>  (top-level)
  //   rules:                --rules <text>       (top-level)
  //   tools:                --tools <csv>        (top-level)
  //   disallowedTools:      --disallowed-tools <csv>      (top-level)
  //   allow[]:              --allow <rule> (repeated)     (top-level)
  //   deny[]:               --deny <rule> (repeated)      (top-level)
  //   worktree (bool|str):  -w [<name>]          (top-level)
  //   sandbox (str):        --sandbox <profile>  (top-level)
  //   alwaysApprove (bool): controls whether `--always-approve` is appended
  //                         to the `agent` subcommand. Default: true.
  _buildArgv() {
    const top = [];
    ...
    // Worktree: accept either a boolean (use `-w`) or a string (use `-w <name>`).
    ...
    // alwaysApprove defaults to true. Only drop the flag when explicitly false.
    const alwaysApprove = s.alwaysApprove === false ? false : true;
    const agentFlags = ['agent', '--no-leader'];
