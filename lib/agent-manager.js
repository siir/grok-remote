...
export class AgentManager extends EventEmitter {
  ...
  async prompt(id, textOrOpts) {
    ...
    // Validate image attachments against the agent's promptCapabilities.
    if (attachments.length) {
      const caps = a.client?.handshake?.agentCapabilities?.promptCapabilities || {};
      const hasImage = attachments.some(att => att && att.kind === 'image');
      if (hasImage && caps.image !== true) {
        const err = new Error('Active model does not support images.');
        err.code = 'IMAGE_UNSUPPORTED';
        throw err;
      }
    }
    ...
    const blocks = [];
    if (text && text.length) blocks.push({ type: 'text', text });
    for (const att of attachments) {
      if (!att) continue;
      if (att.kind === 'image' && typeof att.dataBase64 === 'string' && att.dataBase64) {
        blocks.push({
          type: 'image',
    ...
    // Record the user message in history immediately. Strip large base64
    // payloads from the history record but keep a lightweight summary.
    const histAttachments = attachments.map(att => ({
      kind: att?.kind || 'image',
      mimeType: att?.mimeType || null,
      name: att?.name || null,
      size: att?.size || null,
    }));
    const histData = histAttachments.length ? { text, attachments: histAttachments } : { text };
