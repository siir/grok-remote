...
export class AgentManager extends EventEmitter {
  ...
  async prompt(id, textOrOpts) {
    ...
    // with the FULL ABSOLUTE PATHS so the agent does not have to search; it
    // can pass the path directly to its read_file / terminal tools.
    let finalText = text;
    if (savedFiles.length) {
      const lines = savedFiles.map(f =>
        `- ${f.abs} (${f.mimeType || 'application/octet-stream'}, ${humanSize(f.size)})`);
      const refBlock =
        'The user attached the following files to this message. They have been ' +
        'saved on disk. Use these absolute paths directly with your tools (no need to search):\n' +
        lines.join('\n');
