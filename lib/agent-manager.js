...
export class AgentManager extends EventEmitter {
  ...
  async prompt(id, textOrOpts) {
    ...
    // block listing them; the agent can then open them with its own tools.
    let finalText = text;
    if (savedFiles.length) {
      const lines = savedFiles.map(f =>
        `- ${f.rel} (${f.mimeType || 'application/octet-stream'}, ${humanSize(f.size)})`);
      const refBlock = 'Attached files in this turn:\n' + lines.join('\n');
